...
public class SipManager implements SipListener {
	 ...
	 *             if an axception should occur during the initialization
	 *             process
	 */
	public void start() throws CommunicationsException {
		...
		try {
			...
			((SipCommRouter) sipStack.getRouter()).setOutboundProxy(SIPConfig
					.getOutboundProxy());
		} catch (PeerUnavailableException ex) {
			Log.error("start", ex);
		...
		try {
			...
			while (!successfullyBound) {
				try {
					...
					publicIpAddress = new InetSocketAddress(localAddress,
							localPort);
	...
	synchronized public void stop() throws CommunicationsException {
		...
		if (sipStack != null) {
			...
			for (Iterator<SipProvider> it = sipStack.getSipProviders(); it
					.hasNext();) {
	 ...
	 * 
	 * @throws CommunicationsException
	 *             if an exception is thrown by the underlying stack. The
	...
	private void registrationFailed(RegistrationEvent.Type type) {
		try {
			fireRegistrationFailed(registrarAddress == null ? ""
					: registrarAddress, type);
		} catch (Exception e) {
			Log.error("unregister", e);
		}
	}
	 ...
	 *            the transaction to send the response through.
	 * @param request
	...
	public FromHeader getFromHeader(boolean isNew)
		...
		try {
			...
			if (displayName != null && displayName.trim().length() > 0) {
				...
				fromAddress.setDisplayName(UserCredentials.getUserDisplay());// UserCredentials.getUser());
	 ...
	 * @throws CommunicationsException
	 *             if an exception is thrown while calling
	 ...
	 * 
	 * @param useLocalHostAddress
	 *            specifies whether the SipURI in the contact header should
	 ...
	 * @throws CommunicationsException
	 *             if a ParseException occurs while initially composing the
	 *             FromHeader.
	 */
	public ContactHeader getContactHeader(boolean useLocalHostAddress)
		...
		try {
			...
			if (useLocalHostAddress) {
				contactURI = addressFactory
						.createSipURI(null, UserCredentials.getUserDisplay()
								+ "@"
	 ...
	 * @throws CommunicationsException
	 *             if a ParseException is to occur while initializing the array
	 *             list.
	 */
	public ArrayList<ViaHeader> getLocalViaHeaders()
		...
		try {
			...
			ViaHeader viaHeader = headerFactory.createViaHeader(
					SIPConfig.getIPAddress(), lp.getPort(), lp.getTransport(),
					null);
	 ...
	 *         requests
	 * @throws CommunicationsException
	 *             if MAX_FORWARDS has an invalid value.
	 */
	 ...
	 * @param credentials
	 *            a set of credentials (username and pass)
	 */
	 ...
	 * 
	 * @param listener
	 *            The CommunicationsListener to be added.
	 */
